...
useEffect(() => {
  ...
  async function configureListeners() {
    ...
    try {
      ...
      // Core binding engine helper to avoid duplicate configurations
      const bindMatchRoomListeners = async (targetRoomAddress: string) => {
        if (matchContract) matchContract.removeAllListeners();
        
        matchContract = await getMatchContract(targetRoomAddress);
        ...
        matchContract.on('MoveCommitted', async (committer: string) => {
          ...
          try {
            if (!matchContract) return;
            const p1Struct = await matchContract.player1();
            const p2Struct = await matchContract.player2();

            const bothCommitted = p1Struct.commitment !== ZeroHash && p2Struct.commitment !== ZeroHash;
            ...
            if (bothCommitted) {
              ...
              setLastRound({ playerMove: playedMove, opponentMove: 0, result: 'DRAW' });
              setCurrentScreen('REVEAL_PHASE');
              setScreenMessage('');
            }
          } catch (err) {
            console.error('Failed to parse commitment flags:', err);
          }
        });
        ...
        matchContract.on('RoundDraw', () => {
          const localSavedMove = localStorage.getItem(`match_${stateRef.current.matchAddress}_move`);
          const playedMove = localSavedMove ? parseInt(localSavedMove, 10) : 0;
          setLastRound({ playerMove: playedMove, opponentMove: playedMove, result: 'DRAW' });
          setCurrentScreen('REVEAL_PHASE');
        });

        // 5. Complete Match Terminated
      ...
      // Handle raw assignment detection
      managerContract.on('PlayerAssigned', async (assignedPlayer: string, roomAddress: string) => {
        if (assignedPlayer.toLowerCase() === stateRef.current.playerAddress) {
          const cleanRoomAddr = roomAddress.toLowerCase();
          setMatchAddress(cleanRoomAddr);
          setIsPlayer1(null);
          setCurrentScreen('WAITING_FOR_MATCH');
          setScreenMessage('Assigned to game lobby! Waiting for a challenger...');
          
          // INSTANTLY bind room triggers using the event parameter before state changes
          await bindMatchRoomListeners(cleanRoomAddr);
        }
      });

      // Session Recovery Strategy: If we boot up with a pre-existing room address active
      if (matchAddress) {
        await bindMatchRoomListeners(matchAddress.toLowerCase());

        // Check if the match is already active in memory
        const activeRoom = await getMatchContract(matchAddress);
        const status = Number(await activeRoom.status());
        if (status === 3) {
          const p1Struct = await activeRoom.player1();
          const p2Struct = await activeRoom.player2();
          setIsPlayer1(p1Struct.addr.toLowerCase() === stateRef.current.playerAddress);
          
          if (p1Struct.addr !== '0x0000000000000000000000000000000000000000' && p2Struct.addr !== '0x0000000000000000000000000000000000000000') {
            setCurrentScreen('PICK');
            setScreenMessage('');
          }
        }
